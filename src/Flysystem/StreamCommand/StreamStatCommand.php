...
use Codementality\FlysystemStreamWrapper\Flysystem\Exception\StatFailedException;
use Codementality\FlysystemStreamWrapper\Flysystem\FileData;
use Codementality\FlysystemStreamWrapper\FlysystemStreamWrapperManager;
use TypeError;
...
final class StreamStatCommand
{
    ...
    public static function getStat(FileData $current)
    {
        ...
        $stats['uid'] = $stats[4] = (int) $current->config[FlysystemStreamWrapperManager::UID];
        $stats['gid'] = $stats[5] = (int) $current->config[FlysystemStreamWrapperManager::GID];
    ...
    public static function getRemoteStats(FileData $current): array
    {
        $converter = new PortableVisibilityConverter(
            (int) $current->config[FlysystemStreamWrapperManager::VISIBILITY_FILE_PUBLIC],
            (int) $current->config[FlysystemStreamWrapperManager::VISIBILITY_FILE_PRIVATE],
            (int) $current->config[FlysystemStreamWrapperManager::VISIBILITY_DIRECTORY_PUBLIC],
            (int) $current->config[FlysystemStreamWrapperManager::VISIBILITY_DIRECTORY_PRIVATE],
            (string) $current->config[FlysystemStreamWrapperManager::VISIBILITY_DEFAULT_FOR_DIRECTORIES]
        );
