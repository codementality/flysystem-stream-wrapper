...
final class StreamLockCommand
{
    public static function run(FileData $current, int $operation): bool
    {
        ...
        $store = StoreFactory::createStore((string) $current->config[FlysystemStreamWrapperManager::LOCK_STORE]);
        $lock = new Lock(
            $current->lockKey,
            $store,
            (float) $current->config[FlysystemStreamWrapperManager::LOCK_TTL],
            false
        );
