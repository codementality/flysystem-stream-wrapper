...
final class FileData
{
    ...
    public function setPath(string $path): void
    {
        ...
        if (!class_exists('FlysystemStreamWrapperManager', false)) {
            throw new \LogicException("Unable to load class: $class_name");
        }
        else {
            $this->filesystem = FlysystemStreamWrapperManager::$filesystems[$this->protocol];    
        }
        //$this->filesystem = FlysystemStreamWrapperManager::$filesystems[$this->protocol];
        $this->config = FlysystemStreamWrapperManager::$config[$this->protocol];
    }
    ...
    {
        return (bool) $this->config[FlysystemStreamWrapperManager::IGNORE_VISIBILITY_ERRORS];
    }

    public function emulateDirectoryLastModified(): bool
    {
        return (bool) $this->config[FlysystemStreamWrapperManager::EMULATE_DIRECTORY_LAST_MODIFIED];
    }
}
