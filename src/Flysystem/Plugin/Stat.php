...
class Stat extends AbstractPlugin
{
   ...
   *
   * @return array
   *   The metadata as returned by Filesystem::getMetadata().
   *
   * @throws \League\Flysystem\FileNotFoundException
   *
   ...
   *   A permissions string, such as '644' or 'drw-rw-r--'
   *
   * @return int
   *   The numeric version of the permissions.
   */
  ...
  private function getUid() {
    return \extension_loaded('posix') ? (int) posix_getuid() : (int) getmyuid();
  }

  /**
   * Gets the system group ID.
   */
  private function getGid() {
    return \extension_loaded('posix') ? (int) posix_getuid() : (int) getmygid();
  }
