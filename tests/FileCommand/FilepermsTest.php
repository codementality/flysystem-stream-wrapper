...
class FilepermsTest extends AbstractFileCommandTestCase
{
    use Assert;
    ...
    public function test(): void
    {
        ...
        $this->assertSame(0100644, fileperms($file->flysystem));
    }

    public function testDirectory(): void
    {
        $dir = $this->testDir->createDirectory(true);
        $this->assertSame(040755, fileperms($dir->flysystem));
    }

    public function testFailed(): void
    {
        $file = $this->testDir->createFile();
        $this->assertFalse(@fileperms($file->flysystem));

        $this->expectErrorWithMessage('Stat failed');
        /** @noinspection PhpUnusedLocalVariableInspection */
        $perms = fileperms($file->flysystem);
    }
}
